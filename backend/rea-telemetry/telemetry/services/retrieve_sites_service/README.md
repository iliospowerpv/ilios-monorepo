# Retrieve Sites Service

A service that retrieves a list of available sites using an authorized token from a supported data provider.

The service is deployed as a [Cloud Function](https://console.cloud.google.com/functions/details/us-central1/retrieve_sites_service?project=prj-ilios-telemetry), which is called on-demand by the platform backend.
...
```shell
CLOUD=0 functions-framework --target=retrieve_sites_service --debug
```
