...
def retrieve_sites_service(request: Request) -> Response:
    cloud_logging.setup()

    if request.method != "POST":
        return response.method_not_allowed()

    params = RetrieveSitesServiceRequestParams.from_request(request)

    params_data = params.model_dump()

    logger.info("Retrieving sites from external API", params=params_data)

    return response.success(service.retrieve_sites(params))
