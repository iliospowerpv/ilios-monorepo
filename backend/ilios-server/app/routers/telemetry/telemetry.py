...
async def get_site_telemetry_health(
    ...
    Thin wrapper over ``compute_site_telemetry_health`` (the shared, read-only
    health computation) so the V2 performance-context aggregator can project the
    SAME freshness verdict verbatim instead of re-deriving it. V2-only
    precedence is preserved inside that service: a V2-backed site resolves its
    health entirely from PostgreSQL and never calls BigQuery.
    """
