...
- A few intentional BigQuery holdouts remain on **non-chart, non-aggregation** paths, each legacy-flag-gated to honest N/A by default (NOT leaks, just no V2 fallback yet): `extend_company_sites_with_energy_attributes` (backs the `/{company_id}/sites` table AND `/investor-dashboard/sites`) and `get_devices_last_reported` (backs O&M `/{site_id}/devices` liveness). They return None/empty when `legacy_telemetry_enabled` is off, so a V2 site shows N/A there even though its charts/health are fully V2. Don't mistake "V2-only" (charts/health/readiness/aggregation) for "every O&M read is V2" — these list/liveness helpers are deferred. Note `devices-overview-section` IS already V2 (raw-reading recency).
...
## Honest-N/A applies to EXPECTED only — ACTUAL is coerced to 0 in the O&M chart shape
The "never fabricate, honest N/A" discipline above covers **expected/loss**. **Actual is NOT honest in the O&M `…-chart` pipeline**: because the legacy O&M response schema makes `actual`/`irradiance` (and the KPI `actual_kw`/`cumulative_actual_kw`) **non-optional**, `v2_chart_data.py` 0.0-fills a missing actual/irradiance bucket, `apply_v2_actual_production` sets the current/cumulative actual to `0.0` when there's no bucket, and the legacy BQ-failure branch 0.0-fills all production fields. The FE `ActualProduction` widget compounds this with `actual_kw ?? 0` and a default `weather='Sunny'` (only EXPECTED uses the honest `resolveExpectedState`). The V2-native `/api/telemetry/v2/sites/{id}/series|device-series` pipeline, by contrast, returns honest empty lists / true nulls and carries NO expected.
**Why:** a missing actual rendering as `0` reads as a real outage/zero-production, which is operationally opposite to "no telemetry". This is the integrity gap the perf-context contract design targets (`docs/v2_performance_context_data_contract_and_actual_display_integrity.md`).
**How to apply:** the proper fix is a nullable-everywhere read model (true null for missing actual, `0` only for measured zero, preserve negative tare) — a FE-only `?? 0`→N/A change is safe for the KPI fields (already nullable on the no-data path) but honest actual in the *time-series* requires relaxing the non-optional O&M schema fields; validate every chart consumer first. Don't widen "V2 is honest N/A" to mean actual is honest today.
