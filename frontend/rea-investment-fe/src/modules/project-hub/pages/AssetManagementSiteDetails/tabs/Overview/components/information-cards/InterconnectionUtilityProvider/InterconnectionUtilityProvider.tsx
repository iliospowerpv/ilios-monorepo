...
const ENTITY_ROLE = 'interconnection_utility' as const;
...
>(({ mode, setMode, siteId, data, reflectFormState, portfolioId }, ref) => {
  ...
  const { handleSubmit, formState, control, reset, setValue } = useForm<InterconnectionUtilityProviderFormFields>({
    mode: 'onChange',
    criteriaMode: 'all',
  ...
  const onSubmit: SubmitHandler<InterconnectionUtilityProviderFormFields> = React.useCallback(
    ...
    [notify, queryClient, reset, setMode, updateInterconnectionUtilityProviderDetails, saveEntityRelationship]
  );
            ...
            <FieldCell component="th" scope="row" align={mode === 'view' ? 'right' : 'left'}>
              {mode === 'view' ? (
                ...
                <EntityPicker
                  ...
                  label="Interconnection Utility Entity"
                  role={ENTITY_ROLE}
                  size="small"
                />
              ) : null}
