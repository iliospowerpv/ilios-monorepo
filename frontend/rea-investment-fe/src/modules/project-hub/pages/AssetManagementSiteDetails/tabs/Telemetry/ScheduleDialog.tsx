...
import {
  ...
  type TelemetryCooldown
} from '../../../../../../hooks/useTelemetryCooldown';
import type {
...
interface ScheduleDialogProps {
  ...
  open: boolean;
  onClose: () => void;
  /** Shared per-project manual cooldown (refresh + catch-up) from the parent. */
  cooldown: TelemetryCooldown;
}
...
/**
 * Admin-only dialog for one mapped project/site's automatic telemetry refresh
 * (scheduler) plus bounded historical catch-up (backfill). Opened from the
 * "Automatic Refresh Schedule" button in the Telemetry tab header; rendered only
 * when the caller has confirmed telemetry-admin permission and the site is
 * telemetry-configured. The backend re-enforces both on every call. No
 * credentials or provider tokens are read or shown here.
 *
 * The scheduler state is fetched only while the dialog is open. Manual catch-up
 * shares the parent's per-project cooldown with the header Refresh control, so a
 * recent refresh disables catch-up here (and vice-versa) until it clears.
 */
export const ScheduleDialog: React.FC<ScheduleDialogProps> = ({ siteId, open, onClose, cooldown }) => {
  const [feedback, setFeedback] = useState<Feedback | null>(null);
  const [backfillOpen, setBackfillOpen] = useState(false);

  const scheduler = useSiteScheduler(siteId, { enabled: open });
  ...
  const handleBackfillConfirm = (payload: BackfillReadingsPayload) => {
    backfill.mutate(payload, {
      onSuccess: res => {
        setFeedback(summarizeBackfill(res));
        cooldown.startCooldown(res.cooldown_seconds);
      },
      onError: err => {
        // A 429 means the shared per-project cooldown is still active; show it as
        // a soft warning and arm the countdown rather than a hard failure.
        const retryAfter = parseRetryAfterSeconds(err);
        if (retryAfter !== null) {
          cooldown.startCooldown(retryAfter);
          setFeedback({
            severity: 'warning',
            message: `Telemetry was refreshed recently. Try the catch-up again in ${formatCooldown(retryAfter)}.`
          });
  ...
  let catchUpTooltip: string;
  if (!isMapped) {
    catchUpTooltip = 'Map this project to a telemetry provider first.';
  } else if (isBusy) {
    catchUpTooltip = 'A telemetry sync is already in progress for this site.';
  } else if (isCoolingDown) {
    catchUpTooltip = `Telemetry was refreshed recently. Available again in ${formatCooldown(cooldown.secondsRemaining)}.`;
  } else {
    catchUpTooltip = 'Run a bounded historical catch-up.';
  }
      ...
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Control scheduled telemetry pulls for this project and run a bounded historical catch-up. Enabling records
            the schedule; the platform runs scheduled pulls in the background when telemetry automation is active.
          </Typography>
          ...
          {state && !scheduler.isLoading && (
            <>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 2 }}>
                ...
                <Tooltip title={catchUpTooltip}>
                  <span>
                    <Button
                      variant="outlined"
                      startIcon={<HistoryIcon />}
                      onClick={() => setBackfillOpen(true)}
                      disabled={isBusy || !isMapped || isCoolingDown}
                    >
                      {isCoolingDown
                        ? `Available in ${formatCooldown(cooldown.secondsRemaining)}`
                        : 'Historical catch-up'}
                    </Button>
                  </span>
                </Tooltip>
