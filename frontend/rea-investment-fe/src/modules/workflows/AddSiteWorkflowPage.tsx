...
/**
 * Pilot 2 page for the native Workflow Engine. It starts an `add_site` run, then hands the
 * serialized definition + run to the SAME reusable <Wizard> shell that drives Add Company.
 * The page owns only the API wiring; the engine (server-side) owns orchestration and dispatches
 * the write to the EXISTING site-create endpoint behind a preview -> confirm -> execute
 * handshake. Per replit.md the backend entity stays "Site" — the UI label reads "Project".
 */
const AddSiteWorkflowPage: React.FC = () => {
    ...
    (async () => {
      try {
        ...
        if (axios.isAxiosError(err) && err.response?.status === 403) {
          setError("You don't have permission to add a project.");
        } else {
  ...
  const handleComplete = (result: ExecuteResponse) => {
    notify(result.message || 'Project created successfully.');
    if (result.entity_id) {
      navigate(`/project-hub/projects/${result.entity_id}`);
    } else {
      navigate('/home');
    }
  };
...
export default AddSiteWorkflowPage;
