...
- **Key invariants**: "Site" is the canonical entity (Project == Site is a UI label only); telemetry/baseline/weather math never fabricates values (honest N/A, never 0); the AI Assistant and reconciliation views are strictly read-only; new feature surfaces are additive and flag-gated; **Data Room Templates are immutable snapshots of structure — applying a template is a creation operation, not a synchronization operation, and existing Data Rooms are never silently reconciled to a template.**
