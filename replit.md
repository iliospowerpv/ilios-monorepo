...
- **Telemetry Module**: Project-scoped telemetry for Data Acquisition Systems (DAS) integration, health monitoring, and device mapping. Supports company-scoped DAS providers.
  - **Telemetry V2 (Phase 1, additive backend)**: Parallel architecture under `/api/telemetry/v2/` introducing a DB-backed provider catalog (`telemetry_provider_catalog`), pluggable `ProviderAdapter` abstraction (wraps existing GCP cloud functions), write-only credential storage with redacting log filter, three-state account model (`status` / `credential_status` / `last_sync_status`), and external-site provenance tracking (`telemetry_external_sites`). New permission key `telemetry:admin` with legacy `Settings:edit` fallback; cross-tenant access returns **404** (not 403). Migration `ff18_telemetry_v2_introduce` is purely additive (Identity Integer PKs preserved on every existing table, only CREATE TABLE / ADD COLUMN with safe defaults / non-destructive UPDATE backfills, idempotent `ON CONFLICT` seed). Legacy `/api/telemetry/*` and `das_connections` writes remain unchanged. Phase 1 is backend-only; frontend still uses the legacy flow.
  - **Telemetry V2 (Phase 2, durable credential storage)**: Replaces the Phase 1 in-memory credential store with `GCPSecretManagerCredentialStore`, backed by Google Cloud Secret Manager via the existing `app.helpers.telemetry.secrets_manager.GCPSecretsManager` helper (added an `access_secret_value` method to fetch payloads in-process). Secret naming pattern: `ilios-telemetry-v2-c{company_id}-{token_hex(6)}` — clearly distinguishable from the legacy `ilios-das-c{company_id}-...` set. Credential payload is JSON-encoded `{field_name: value}`. Credential rotation calls `add_secret_version` against the existing secret resource (previous versions retained for audit/rollback). Backend is selected at process boot via `_build_default_store`: respects `TELEMETRY_V2_CREDENTIAL_BACKEND` env var (`gcp` | `in-memory`), otherwise auto-picks GCP if `service_account_key_file_path` and `gcp_project_id` are configured and the key file exists, falling back to in-memory with a startup warning. Archive endpoint NO LONGER deletes the underlying secret (safe-deletion policy: archive is reversible, secret retained until an explicit purge endpoint is added). Audit-grade structured logs on create/rotate/archive (`telemetry_v2_credential_*`, `telemetry_v2_account_*`) — log filter scrubs values. Restart-durability validation script at `tests/scripts/phase2_durability_validation.py` (18/18 checks: store/retrieve/rotate/archive across simulated process restart, redaction across whole flow). Phase 2 has **zero schema changes** — pure storage refactor on top of existing ff18 tables.
  - **Demo Telemetry System** with database-driven `is_demo` flag on companies, toggled via `DEMO_TELEMETRY=true` env var. Demo companies' sites receive realistic simulated solar data (bell-curve production, seasonality, weather variability, degradation) with injected demo events (inverter outage, clipping, severe weather, intermittent faults). Data flows through the same pipeline as live telemetry (intercepted at `BaseTelemetryBigQuery` level). Non-demo companies are completely unaffected. Demo telemetry infrastructure (DAS connection, site mappings, device mappings) is seeded via `scripts/seed_demo_telemetry.py` (seed/cleanup). Device power uses DB-derived inverter count per site for correct capacity scaling.
